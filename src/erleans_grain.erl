...
call(GrainRef, Request, Timeout) ->
    ...
    do_for_ref(GrainRef, fun(_, Pid) ->
                                 try
                                     gen_statem:call(Pid, {otel_tracer:current_span_ctx(opentelemetry:get_tracer()), ReqType, Request}, Timeout)
                                 catch
                                     exit:{bad_etag, _} ->
...
cast(GrainRef, Request) ->
    ReqType = req_type(),
    do_for_ref(GrainRef, fun(_, Pid) -> gen_statem:cast(Pid, {otel_tracer:current_span_ctx(opentelemetry:get_tracer()), ReqType, Request}) end).
...
active({call, From}, {SpanCtx, ReqType, Msg}, Data=#data{cb_module=CbModule,
                                                         cb_state=CbData,
                                                         deactivate_after=DeactivateAfter}) ->
    otel_tracer:start_span(opentelemetry:get_tracer(), span_name(Msg), #{parent => SpanCtx}),
    otel_tracer:set_attribute(<<"grain_msg">>, io_lib:format("~p", [Msg])),
    try handle_result(CbModule:handle_call(Msg, From, CbData), Data, upd_timer(ReqType, DeactivateAfter))
    after
        otel_tracer:end_span(opentelemetry:get_tracer())
    end;

...
-define(PDB_PREFIX, {erlans_pm, grain_ref}).
...
stale_local_entry(Config) ->
    ...
    plum_db:put(?PDB_PREFIX, GrainRef, [partisan:self()]),

    ?assertMatch(
        undefined,
        erleans_pm:whereis_name(GrainRef),
        "Should not exist as its a remote pid and there is no entry "
        "in the local erleans_pm ets table."
    ).
